<template>
  <div class="settings-container">
    ...
    <el-scrollbar class="scroll" v-if="!firstLoading">
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              <div class="setting-item">
                <div>
                  <span>主用 AI 提供商</span>
                  <el-tooltip effect="dark" content="Workers AI 免费（Cloudflare 免费额度 1 万 Neurons/天）；DeepSeek/自定义按量付费">
                    <Icon class="warning" icon="fe:warning" width="18" height="18"/>
                  </el-tooltip>
                </div>
                <div class="forward">
                  <el-tag :type="aiStatusInfo.fallback ? 'warning' : 'success'" size="small">
                    {{ aiStatusInfo.primaryLabel || '未配置' }}
                  </el-tag>
                  <el-button class="opt-button" size="small" type="primary" @click="openAiPrimarySetting">
                    <Icon icon="fluent:settings-48-regular" width="18" height="18"/>
                  </el-button>
                </div>
              </div>
              <div class="setting-item">
                <div>
                  <span>自动降级</span>
                  <el-tooltip effect="dark" content="仅当主用为 Workers AI 时生效；调用失败时自动切换到备用模型">
                    <Icon class="warning" icon="fe:warning" width="18" height="18"/>
                  </el-tooltip>
                </div>
                <div class="forward">
                  <span>{{ setting.aiFallbackStatus === 0 ? $t('enabled') : $t('disabled') }}</span>
                  <el-button class="opt-button" size="small" type="primary" @click="openAiFallbackSetting">
                    <Icon icon="fluent:settings-48-regular" width="18" height="18"/>
                  </el-button>
                </div>
              </div>
              <div class="setting-item" v-if="setting.aiFallbackStatus === 0">
                <div><span>备用模型</span></div>
                <div class="forward">
            ...
            <div class="card-content">
              <div class="concerning-item">
                ...
                <el-badge is-dot :hidden="!hasUpdate">
                  <el-button @click="jump('https://github.com/cunzhangcrypto/CunInbox/releases')">
                    {{ currentVersion }}
                    <template #icon>
              ...
              <div class="concerning-item">
                ...
                <div class="community">
                  <el-button @click="jump('https://github.com/cunzhangcrypto/CunInbox')">
                    Github
                    <template #icon>
                      <Icon icon="codicon:github-inverted" width="22" height="22"/>
                    </template>
                  </el-button>
                  <el-button @click="jump('https://t.me/cunzhangtech')">
                    Telegram
                    <template #icon>
      ...
      <!-- AI 主用配置对话框 -->
      <el-dialog v-model="aiPrimaryShow" title="主用 AI 配置" width="420">
        <div class="ai-config-form">
          <div class="ai-form-item">
            <div class="ai-form-label">AI 提供商</div>
            <el-select v-model="aiPrimaryForm.aiProvider" style="width: 100%">
              <el-option :value="0" label="Workers AI (Cloudflare 免费)" />
              <el-option :value="1" label="DeepSeek" />
              <el-option :value="2" label="自定义 (OpenAI 兼容)" />
            </el-select>
          </div>
          <template v-if="aiPrimaryForm.aiProvider !== 0">
            <div class="ai-form-item">
              <div class="ai-form-label">API Base URL</div>
              <el-input v-model="aiPrimaryForm.aiBaseUrl" placeholder="https://api.deepseek.com" />
            </div>
            <div class="ai-form-item">
              <div class="ai-form-label">API Key</div>
              <el-input v-model="aiPrimaryForm.aiApiKey" type="password" show-password
                        placeholder="留空表示不修改（已配置的 Key 已脱敏显示）" />
            </div>
            <div class="ai-form-item">
              <div class="ai-form-label">模型名</div>
              <el-input v-model="aiPrimaryForm.aiModel" placeholder="deepseek-chat" />
            </div>
          </template>
          <div class="ai-form-tip" v-if="aiPrimaryForm.aiProvider === 0">
            Workers AI 每天免费 10,000 Neurons（约 UTC 00:00 重置）。免费额度耗尽当天 AI 功能将不可用，建议同时启用「自动降级」。
          </div>
        </div>
        <template #footer>
          <el-button @click="aiPrimaryShow = false">取消</el-button>
          <el-button type="primary" :loading="settingLoading" @click="saveAiPrimary">保存</el-button>
        </template>
      </el-dialog>

      <!-- AI 降级配置对话框 -->
      <el-dialog v-model="aiFallbackShow" title="AI 自动降级配置" width="420">
        <div class="ai-config-form">
          <div class="ai-form-tip" style="margin-bottom: 14px;">
            仅当主用 AI 为 Workers AI 时生效。Workers AI 调用失败（如免费额度耗尽、模型不可用）时，自动切换到下方备用模型。
          </div>
          <div class="ai-form-item">
            <div class="ai-form-label">启用自动降级</div>
            <el-switch v-model="aiFallbackForm.aiFallbackStatus" :active-value="0" :inactive-value="1" />
          </div>
          <template v-if="aiFallbackForm.aiFallbackStatus === 0">
            <div class="ai-form-item">
              <div class="ai-form-label">备用 API Base URL</div>
              <el-input v-model="aiFallbackForm.aiFallbackBaseUrl" placeholder="https://api.deepseek.com" />
            </div>
            <div class="ai-form-item">
              <div class="ai-form-label">备用 API Key</div>
              <el-input v-model="aiFallbackForm.aiFallbackApiKey" type="password" show-password
                        placeholder="留空表示不修改（已配置的 Key 已脱敏显示）" />
            </div>
            <div class="ai-form-item">
              <div class="ai-form-label">备用模型名</div>
              <el-input v-model="aiFallbackForm.aiFallbackModel" placeholder="deepseek-chat" />
            </div>
          </template>
        </div>
        <template #footer>
          <el-button @click="aiFallbackShow = false">取消</el-button>
          <el-button type="primary" :loading="settingLoading" @click="saveAiFallback">保存</el-button>
        </template>
      </el-dialog>
    </el-scrollbar>
  </div>
...
import {aiStatus as aiStatusApi} from "@/request/ai.js";
...
// AI 配置
const aiPrimaryShow = ref(false)
const aiFallbackShow = ref(false)
const aiStatusInfo = ref({
  provider: 0, primaryLabel: '', hasWorkersAI: false,
  fallbackEnabled: false, fallbackLabel: ''
})
const aiPrimaryForm = reactive({
  aiProvider: 0, aiApiKey: '', aiBaseUrl: 'https://api.deepseek.com', aiModel: 'deepseek-chat'
})
const aiFallbackForm = reactive({
  aiFallbackStatus: 1, aiFallbackApiKey: '',
  aiFallbackBaseUrl: 'https://api.deepseek.com', aiFallbackModel: 'deepseek-chat'
})
...
function loadAiStatus() {
  aiStatusApi().then(res => {
    aiStatusInfo.value = res || {}
  }).catch(() => null)
}

function openAiPrimarySetting() {
  aiPrimaryForm.aiProvider = setting.value.aiProvider ?? 0
  aiPrimaryForm.aiBaseUrl = setting.value.aiBaseUrl || 'https://api.deepseek.com'
  aiPrimaryForm.aiModel = setting.value.aiModel || 'deepseek-chat'
  aiPrimaryForm.aiApiKey = ''
  aiPrimaryShow.value = true
}

function openAiFallbackSetting() {
  aiFallbackForm.aiFallbackStatus = setting.value.aiFallbackStatus ?? 1
  aiFallbackForm.aiFallbackBaseUrl = setting.value.aiFallbackBaseUrl || 'https://api.deepseek.com'
  aiFallbackForm.aiFallbackModel = setting.value.aiFallbackModel || 'deepseek-chat'
  aiFallbackForm.aiFallbackApiKey = ''
  aiFallbackShow.value = true
}

async function saveAiPrimary() {
  if (settingLoading.value) return
  const form = { aiProvider: aiPrimaryForm.aiProvider }
  if (aiPrimaryForm.aiProvider !== 0) {
    form.aiBaseUrl = aiPrimaryForm.aiBaseUrl
    form.aiModel = aiPrimaryForm.aiModel
    if (aiPrimaryForm.aiApiKey) form.aiApiKey = aiPrimaryForm.aiApiKey
  }
  editSetting(form, true)
  loadAiStatus()
}

async function saveAiFallback() {
  if (settingLoading.value) return
  const form = {
    aiFallbackStatus: aiFallbackForm.aiFallbackStatus,
    aiFallbackBaseUrl: aiFallbackForm.aiFallbackBaseUrl,
    aiFallbackModel: aiFallbackForm.aiFallbackModel
  }
  if (aiFallbackForm.aiFallbackApiKey) form.aiFallbackApiKey = aiFallbackForm.aiFallbackApiKey
  editSetting(form, true)
  loadAiStatus()
}
...
function getUpdate() {
  ...
  axios.get('https://api.github.com/repos/cunzhangcrypto/CunInbox/releases/latest').then(({data}) => {
...
function editSetting(settingForm, refreshStatus = true) {
  ...
  settingSet(settingForm).then(() => {
    ...
    aiPrimaryShow.value = false
    aiFallbackShow.value = false
  }).catch((e) => {
    loginOpacity.value = setting.value.loginOpacity
