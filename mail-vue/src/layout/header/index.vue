<template>
  <div class="header" :class="!hasPerm('email:send') ? 'not-send' : ''">
    ...
    <div class="toolbar">
      <a href="https://czlab.dev" target="_blank" rel="noopener noreferrer" class="czlab-btn" title="村长实验室">
        <Icon icon="mdi:flask-outline" width="18" height="18"/>
        <span class="czlab-text">村长实验室</span>
      </a>
        ...
        <template #dropdown>
          <div class="user-details">
            ...
            <div class="detail-user-type">
              <el-tag>{{ userStore.user?.role?.name || 'USER' }}</el-tag>
            </div>
...
const sendType = computed(() => {
  ...
  if (!role) return t('unauthorized')

  if (role.sendType === 'ban') {
    return t('sendBanned')
  }

  if (role.sendType === 'internal') {
  ...
  if (role.sendType === 'day') {
    return t('daily')
  }

  if (role.sendType === 'count') {
    return t('total')
  }
})
...
.toolbar {
  ...
  .czlab-btn:hover {
    border-color: var(--ci-cyan, #00e5ff);
