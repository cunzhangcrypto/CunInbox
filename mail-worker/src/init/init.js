...
const dbInit = {
	...
	async v3DB(c) {
		const ADD_COLUMN_SQL_LIST = [
			`ALTER TABLE account ADD COLUMN alias_type INTEGER NOT NULL DEFAULT 0;`,
			`ALTER TABLE account ADD COLUMN identity_id INTEGER NOT NULL DEFAULT 0;`,
			`ALTER TABLE account ADD COLUMN alias_label TEXT NOT NULL DEFAULT '';`,
			`ALTER TABLE setting ADD COLUMN ai_provider INTEGER NOT NULL DEFAULT 0;`,
			`ALTER TABLE setting ADD COLUMN ai_api_key TEXT NOT NULL DEFAULT '';`,
			`ALTER TABLE setting ADD COLUMN ai_base_url TEXT NOT NULL DEFAULT 'https://api.deepseek.com';`,
			`ALTER TABLE setting ADD COLUMN ai_model TEXT NOT NULL DEFAULT 'deepseek-chat';`,
			`ALTER TABLE setting ADD COLUMN ai_analysis_status INTEGER NOT NULL DEFAULT 1;`,
			`ALTER TABLE setting ADD COLUMN ai_fallback_status INTEGER NOT NULL DEFAULT 0;`,
			`ALTER TABLE setting ADD COLUMN ai_fallback_api_key TEXT NOT NULL DEFAULT '';`,
			`ALTER TABLE setting ADD COLUMN ai_fallback_base_url TEXT NOT NULL DEFAULT 'https://api.deepseek.com';`,
			`ALTER TABLE setting ADD COLUMN ai_fallback_model TEXT NOT NULL DEFAULT 'deepseek-chat';`
		];
		...
		const CREATE_TABLE_SQL_LIST = [
			...
			`CREATE TABLE IF NOT EXISTS email_analysis (
				...
				is_new_identity INTEGER NOT NULL DEFAULT 0,
		...
		for (const sql of CREATE_TABLE_SQL_LIST) {
			try {
				await c.env.db.prepare(sql).run();
			} catch (e) {
		...
		try {
			await c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_email_analysis_email_id ON email_analysis(email_id);`).run();
			await c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_identity_user_id ON identity(user_id);`).run();
			await c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_security_event_user_id ON security_event(user_id);`).run();
			await c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_daily_digest_user_date ON daily_digest(user_id, digest_date);`).run();
		} catch (e) {
			console.warn(`跳过创建索引：${e.message}`);
		}

		const { platformCount } = await c.env.db.prepare(`SELECT COUNT(*) as platformCount FROM platform`).first();
		if (platformCount === 0) {
			await c.env.db.prepare(`
				INSERT INTO platform (name, domain, category, description, official_url, is_system) VALUES
				('GitHub', 'github.com', 'dev', '代码托管平台', 'https://github.com', 1),
				('GitLab', 'gitlab.com', 'dev', 'DevOps平台', 'https://gitlab.com', 1),
				('Cloudflare', 'cloudflare.com', 'cloud', '边缘云服务', 'https://cloudflare.com', 1),
				('Vercel', 'vercel.com', 'cloud', '前端部署平台', 'https://vercel.com', 1),
				('OpenAI', 'openai.com', 'ai', 'AI平台', 'https://openai.com', 1),
				('Anthropic', 'anthropic.com', 'ai', 'AI平台', 'https://anthropic.com', 1),
				('Google', 'google.com', 'saas', '搜索引擎服务', 'https://google.com', 1),
				('AWS', 'aws.amazon.com', 'cloud', '云服务', 'https://aws.amazon.com', 1),
				('Microsoft', 'microsoft.com', 'saas', '软件服务', 'https://microsoft.com', 1),
				('Bilibili', 'bilibili.com', 'social', '视频平台', 'https://bilibili.com', 1),
				('Twitter', 'twitter.com', 'social', '社交平台', 'https://twitter.com', 1),
				('Telegram', 'telegram.org', 'social', '即时通讯', 'https://telegram.org', 1),
				('Stripe', 'stripe.com', 'finance', '支付平台', 'https://stripe.com', 1),
				('PayPal', 'paypal.com', 'finance', '支付平台', 'https://paypal.com', 1)
			`).run();
		}
	},
	...
	async v1_6DB(c) {
		...
		const ADD_COLUMN_SQL_LIST = [
			...
			`ALTER TABLE setting ADD COLUMN notice_title TEXT NOT NULL DEFAULT 'CunInbox';`,
